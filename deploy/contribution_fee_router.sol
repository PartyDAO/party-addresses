...
contract DeployScript is Script, Deploy {
    ...
    function deploy(LibDeployConstants.DeployConstants memory deployConstants) public override {
        ...
        addressMapping[0] = AddressMapping("AllowListGateKeeper", address(allowListGateKeeper));
        addressMapping[1] = AddressMapping("TokenGateKeeper", address(tokenGateKeeper));
        addressMapping[2] = AddressMapping("ContributionRouter", address(contributionRouter));
